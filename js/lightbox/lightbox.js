			...
			**/
			...
			if (!opts.jsonData) {
				...
				if ((!imageObject.rel || (imageObject.rel == '')) && !opts.allSet) {
					...
					opts.imageArray.push(new Array(imageObject.href, opts.displayTitle ? (imageObject.title ? imageObject.title : imageObject.getAttribute('original-title') ) : ''));
				} else {
			...
			var lightboxTop = arrayPageScroll[1] + ($(window).height() / 20); // 10 -20
		...
		function doChangeImage() {
			...
			imgPreloader.onload = function() {
				...
				if (opts.fitToScreen) {
					...
					var initialPageHeight = arrayPageSize[3] - 120; // - 200
