<?php if(!defined('cfih') or !cfih) die('This file cannot be directly accessed.');
    ...
    class Ico {
        ...
        function &GetIcon($index=null) {
			// added to get the bigs icon
			if(is_null($index)){
				$top_size = $top_bit = 0;
				foreach( $this->formats as $k => $ico_s){
					if(($ico_s['Width'] > $top_size) || ($ico_s['Width'] == $top_size && $ico_s['BitCount'] > $top_bit )){
						$top_size = $ico_s['Width'];
						$top_bit = $ico_s['BitCount'];
						$index = $k;
					}
				}
			}
                    ...
                    for ($i = $this->formats[$index]['Height'] - 1; $i >= 0; $i--) {
                        for ($j = 0; $j < $this->formats[$index]['Width']; $j++) {
                            if ($leftbits) {
								...
								//$color = ord($this->formats[$index]['data'][$offset]);
                                $color = array(
