...
// added by codefuture
	function Off_set(){
		foreach ($this->GIF_arrays as $key => $value) {
			$this->pointer =10;
			if($this->readbits(($mybyte=$this->readbyte_int($value)),0,1)==1){
				$this->pointer +=2;
				$this->pointer +=(pow(2,$this->readbits($mybyte,5,3)+1)*3);
			}else{
				$this->pointer +=2;
			}
			$value = substr($value,$this->pointer+9);
			$this->offset[] = array("offset_left" => $this->dualbyteval(substr($value,1,2)),
									"offset_top" => $this->dualbyteval(substr($value,3,2))
									);
		}
		return $this->offset;
	}

	private function readbyte_int($v){
		$data = substr($v,$this->pointer+1);
		return ord($data);
	}
...
// added to help debug
	public function debug_this() {
		foreach ($this as $key => $value) {
			echo '<br/><b>'.$key.'</b><br/><pre>';
			var_dump($value);
			echo '</pre><hr/>';
		}
	} 
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::
::	GIFEncoder Version 3.0 by L·szlÛ Zsidi, http://gifs.hu
::
